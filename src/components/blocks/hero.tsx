  ...
  (
    ...
    return (
      <section
        ref={ref}
        className={cn(
          "relative z-0 flex min-h-[calc(100vh-theme(spacing.20))] w-full flex-col items-center justify-center overflow-hidden bg-background", // Adjusted min-h to account for header
          className,
        )}
        {...props}
      >
        ...
        <motion.div
          ...
          className="relative z-50 container flex justify-center flex-1 flex-col px-5 md:px-10 gap-4 -translate-y-10 md:-translate-y-20" // Adjusted translate-y
        >
          <div className="flex flex-col items-center text-center space-y-4">
            ...
            {subtitle && (
              <p
                className={cn(
                  "text-lg md:text-xl text-muted-foreground max-w-2xl mx-auto", // Added max-w and mx-auto
                  subtitleClassName,
                )}
              >
                {subtitle}
            ...
            {actions && actions.length > 0 && (
              <div className={cn("flex flex-col sm:flex-row gap-4 mt-8", actionsClassName)}> {/* Added mt-8 */}
